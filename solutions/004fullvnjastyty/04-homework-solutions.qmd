...
library(tidyverse)
library(here)
survey_df <- read_csv(here("data_private/stat113_survey_f26.csv"))
...
c. $\hat{p} =$ `r (survey_sum$totalright / survey_sum$total) |> round(3)`.

d. Can make this argument either way. I would argue it probably is representative because, though it is a convenience sample, students who choose not to take STAT 113 probably have similar handedness characteristics as students who choose to take STAT 113. 
...
a. $\hat{p} =$ `r (survey_sum$totalright / survey_sum$total) |> round(3)`.

b. Check to see if $n \cdot \hat{p}$ is greater than 10 and $n \cdot (1 - \hat{p})$ is greater than 10. If either `r (survey_sum$total * (survey_sum$totalright / survey_sum$total)) |> round(3)` or  `r (survey_sum$total * (1 - survey_sum$totalright / survey_sum$total)) |> round(3)` is less than 10, then the assumption does not hold.
...
c. (`r ci_hand[1]`, `r ci_hand[2]`).

d. We are 99% confident that the proportion of students at SLU who are right-handed is between `r ci_hand[1]` and `r ci_hand[2]`.
